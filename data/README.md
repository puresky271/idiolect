...
3. **画像不导出 `anchor_density`**（2026-09-13 复审定死）：composite 的 anchor_ref 唯一来源是 `scene_char_baseline.json` 的派生值（`docs/04-evaluation.md` 第 2.1 节）。画像字段曾短暂存在，因与派生值构成双口径（乐奈差 28%）而移除——别把它加回来。
