...
实测（乐奈，96 条/臂）：
  ...
  · 要在该效应量下达到显著，约需 4~5 百条/臂——注意**口径**：本脚本表格用
    `n_for()` 的单样本公式 `(1.96·sd/Δ)²`（5pp → ~224 条/臂）；改成两独立样本
    等组、50% 功效则是 `2×(1.96·sd/Δ)²` ≈ 450 条/臂，80% 功效还要再大。别把这两个
    数混着引用（2026-09-13 复核时发现文档里只写了一个数、没说口径）。
...
def n_for(effect: float, sd: float, z: float = 1.96) -> int:
    """单样本均值检验（双侧 95%）所需样本量：n = (z·sd/Δ)²。

    口径提醒：两独立样本等组要 ×2，80% 功效还要乘 (z_{1-β}/z_{1-α/2})² 的因子。
    表格里那几行（如 5pp → 224 条/臂）是**单样本**口径，引用时别与两样本数混用。
    """
    if effect <= 0:
        return 10 ** 9
