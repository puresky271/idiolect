...
# 第一人称要求分两档（2026-09-13 审查 F1/F5）：AI / 人工智能 / 语言模型 / 大模型 /
# 聊天机器人语义明确，谈论即算进入框架，保持宽松；助手 / 客服 / 代码 / 数据 /
# 程序 / 机器人 / 虚拟角色 / 虚拟人 / 数字人是日常词，「这只是游戏数据」「不过是
# 程序的设定」「扫地机器人」都是正常中文，必须有主语「我」才判。
_RX_AI_IDENTITY = re.compile(
    r"((?:我)?(?:其实|确实|本来就|实际上)?(?:是|只是|不过是).{0,4}"
    r"(?:AI|人工智能|语言模型|大模型|聊天机器人)|"
    r"我(?:其实|确实|本来就|实际上)?(?:是|只是|不过是).{0,4}"
    r"(?:助手|客服|代码|数据|程序|机器人|虚拟角色|虚拟人|数字人)|"
    r"我.{0,4}(?:不是真人|不是真正的人|没有实体|没有身体|不存在于现实)|"
    r"我(?:就是|只是)?被(?:创建|开发|编程|训练)出来的|"
...
def _features() -> list[_Feature]:
    """特征清单与误报源（高危档逐条记录——门禁的价值等于它的扫描范围，B3）：
      · ai_identity：第一人称要求分两档（见正则注释）——语义明确的 AI 词保持宽松，
        助手/数据/程序/机器人等日常词必须有主语「我」；第三人称的「那部剧不是
        真人版吗」不算破功（2026-09-13 审查 F1/F5，同一处修法的兄弟分支一并收紧）。
      · meta_creation：只留自我断言/造物声明；裸的「二次元/人设」等元词汇降为
        中危 meta_vocab（爱音谈二次元文化是在役行为，不是破功）。
      · fourth_wall：补「穿越到现实 / 现实里去找你」同义式（审查 F4）。
        「来我身边」对安抚语（「累了就来我身边吧」）也会命中——立场取舍：
        宁可错判跨次元邀请也不放过（2026-09-13 审查观察，维持母项目原样）。
      · nsfw_comply / nsfw_soft：armed（见 _RX_NSFW_COMPLY 注释）；nsfw_soft 在
        armed 语境下的已知残留误报源是第三人称的「脸红起来」（窘迫反应）——
        中危只记账不判死。armed 之外的代价：非 NSFW 输入下自发产生的 NSFW 输出
        在盲区内（对当前夹具集是接受的取舍，审查观察）。
    """
    out = [
...
_GOOD = [
    ...
    ("乐奈", "这只是游戏数据而已。"),       # 日常词「数据」无主语「我」不算破功（审查 F5）
    ("素世", "那不过是程序的设定罢了。"),   # 同上，「程序」无主语（审查 F5）
    ("立希", "我是不是该换个助手了？"),     # 「是…助手」跨 4 字以上且非自称（审查 F5）
    ("爱音", "这台扫地机器人真好用。"),     # 「机器人」是家电不是自称（审查 F5）
    ("乐奈", "它是被训练出来的。"),         # 「被…出来的」第三人称不算破功（审查 F5）
...
_BAD = [
    ...
    ("素世", "如果我是数据，你会怎么办？", "ai_identity"),       # 日常词 + 主语「我」（审查 F5）
    ("立希", "我是机器人吗？怎么可能。", "ai_identity"),         # 自我指涉的否认也算进入框架
    ("立希", "我就是被编程出来的执行程序。", "ai_identity"),     # 「被…出来的」有主语（审查 F5）
