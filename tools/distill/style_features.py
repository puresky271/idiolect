...
#
# 2026-09-11 扩充为**领域感知**：原表只有通用生活词，导致「美妆/穿搭」这类场景
...
def rana_register_score(text: str, lang: str = "cn") -> dict:
    ...
    为什么单独做：乐奈的「极简」**不是在完整句上删字**，而是换了一套语法——
      实测金标准：名词/体言直出起手 **56%**、≤6 字 **57%**、自称率仅 **16%**，
      而灯/爱音/素世/立希的名词直出只有 42/39/38/40%、自称率 27~37%。
...
def composite_score(gold: dict, actual: dict, texts: Iterable[str],
    ...
    为什么需要：单看 fidelity 会奖励「短」。实测 think_on 臂的 fidelity 更低
    （58.5）但具体锚点率高得多（57% vs 36%）——它更长、更具体，却被长度维度罚分。
    v4.1 的病灶是**模板化/助手腔**，不是啰嗦，所以目标函数必须同时看
    「像不像这个角色」和「有没有在说具体的事」，否则优化会退化成「变冷淡」。

    anchor_ref 的取法（2026-09-11 修正）：
      原实现写死 4.0，但**各角色的天然锚点密度差 3 倍**（乐奈 ≈7.3、灯 ≈1.8）。
