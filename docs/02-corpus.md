...
> 用于重算统计或迁移到其他作品：先确认语料来源与切分，再运行审计和导出。仅体验五人 Skill 或使用已发布画像时，无需重新抓取语料。

**目录**：[1 语料是什么](#1-语料是什么) ｜ [2 抓取与清洗链路](#2-抓取与清洗链路) ｜ [3 质量审计](#3-质量审计) ｜ [4 派生统计清单](#4-派生统计清单) ｜ [5 落地操作](#5-落地操作) ｜ [6 边界与限制](#6-边界与限制)
...
实跑通过：`analyze_corpus`、`verbal_tics`、`tic_profile`、`char_topic_vocab`、`export_targets`、`verify_triggers`、`export_scene_targets`、`export_profiles`、`scene_char_baseline`、`scene_stats`、`tic_by_scene`、`validate_scenes`、`audit_corpus_quality`、`probe_registry --list`、`power_calc`。另外在**新克隆的仓库**里跑通了 `pytest tests`（未配置语料时，相关项按预期跳过）、`offline_smoke` 与探针 dry-run（35 条记录，system 段 12368~24501 字符，无需语料）。

仍未验证的两项，都因为需要外部资源：

- `tools/corpus/crawl_bestdori.py`：要访问 bestdori.com。它的文件头声称产出行含 `source`，与 `bd_api.extract_lines` 的实际字段不符（`source` 是 `build_gold.py` 事后打的标签）。
