...
**不允许可整句照抄的例句。** 实测：注入正文里放正例台词时，生产臂 22%~27% 的回复逐字复述了正文里的句子，最长公共子串 22 字；`rana.cat_talk` 的 6 条回复里 5 条完全相同（都等于模块里的正例）。
...
长度控制改由**字数上限 + 句数**承担。代价是实测过的：零样本用词之后长度贴合度从 0.512 掉到 0.461，这个代价是自愿接受的。
