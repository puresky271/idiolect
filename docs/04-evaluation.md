...
读池化结果的方式：看**改善格数与退化格数的比例**，再看池化均值差。当前 20 条夹具上的一次池化结果：7 个注入臂的均值从 0.326 到 0.483（Δ +0.157，20 格里 15 格改善）；另外 15 条新夹具上 Δ −0.004（8/15 改善）。前者是「改动方向对」，后者是「换一批夹具就没了」，两句话必须一起说。
...
`IDIOLECT_MOCK_NOW=real` 会走真实时钟，此时探针会打印警告：这批数字不能与 mock 批次比较。
...
判定标准：用同一份语料重跑生成脚本，产物与仓库里的文件**逐字节相同**。2026-09-13 实测六个文件全部满足（六条命令见 `data/README.md`）。为了达到这个标准修掉两处不确定性（词表并列项、top-N 截断的兜底排序）。
